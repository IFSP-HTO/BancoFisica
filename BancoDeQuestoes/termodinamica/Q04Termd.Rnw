...
plot(x, y, type="o", pch=20, col="black", ylab="Pressão (10^3 Pa)", ylim=range(0, Pmax), yaxp=c(0,Pmax,4), xlim=range(0, xpasso), xlab="Volume (10^-4 m³)", xaxp=c(0,xpasso,5))
...
\end{figure}
  ...
  \item O trabalho realizado durante a transformação de A para B. \textit{Caso necessário, arredonde sua resposta resposta para uma casa decimal}

  \item A variação da energia interna do sistema durante a transformação de A para B. \textit{Caso necessário, arredonde sua resposta resposta para uma casa decimal}

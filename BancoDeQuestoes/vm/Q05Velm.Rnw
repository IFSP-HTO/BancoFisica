...
# 13/08/2017 questão modificada por vinibrenobr11
...
questions <- solutions <- explanations <- rep("", 3)
type <- rep(c("schoice"), 3)

questions[1] <- ""
solutions[1] <- explanations[1] <- floor(resp)
type[1] <- "num"

questions[2] <- ""
solutions[2] <- explanations[2] <- round((resp - floor(resp)) * 60)
type[2] <- "num"

resp <- (resp - floor(resp)) * 60

questions[3] <- ""
solutions[3] <- explanations[3] <- round((resp - floor(resp)) * 60)
type[3] <- "num"

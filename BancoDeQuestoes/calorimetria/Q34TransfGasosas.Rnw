...
%% SOLUTION
\begin{solution}
Inicialmente, o gás ocupa apenas o balão \(A\), de volume \(V_A=\Sexpr{vol1}\,\mathrm{L}\). Após abrir a torneira, ele ocupa o volume total
\[
  V_f=V_A+V_B
     =\Sexpr{vol1}+\Sexpr{vol2}
     =\Sexpr{vol1+vol2}\,\mathrm{L}.
\]

Como a temperatura final retorna ao valor inicial e a quantidade de gás é a mesma,
\[
  p_iV_i=p_fV_f.
\]
Logo,
\[
  p_f=
  \frac{\Sexpr{nota_cient(p1)}\cdot\Sexpr{vol1}}
       {\Sexpr{vol1+vol2}}
  \,\mathrm{Pa}.
\]

Usando \(1\,\mathrm{atm}=101000\,\mathrm{Pa}\),
\[
  p_f=
  \frac{\Sexpr{p1}\cdot\Sexpr{vol1}}
       {101000\,(\Sexpr{vol1+vol2})}
  =\Sexpr{resp}\,\mathrm{atm}.
\]

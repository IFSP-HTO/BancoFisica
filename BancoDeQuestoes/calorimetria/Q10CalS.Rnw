...
y <- c(Tmin, Tmax)
x <- c(0, tmax)
plot(x, y, type="o", col="blue", ylab=" Temperatura \U00BA C)", ylim=range( Tmin,Tmax), yaxp=c(Tmin,Tmax,2), xlab="Tempo (min)", xaxp=c(0,tmax,5))
abline(h=c(Tmin,(Tmax+Tmin)/2,Tmax), v=seq(from=0, to=tmax, by=tmax/5), col="gray", lty=3)
@
\end{center}
\end{figure}

Determine o calor específico da substância em $cal/g^\circ C$. Dê sua resposta com duas casas decimais.

...
\begin{solution}
A fonte fornece calor a uma taxa constante de \(\Sexpr{P}\,\mathrm{cal/min}\) durante \(\Sexpr{tmax}\,\mathrm{min}\). Logo,
\[
  Q=\Sexpr{P}\cdot\Sexpr{tmax}
   =\Sexpr{P*tmax}\,\mathrm{cal}.
\]

Pelo gráfico, a temperatura varia de \(\Sexpr{Tmin}\,^{\circ}\mathrm{C}\) para \(\Sexpr{Tmax}\,^{\circ}\mathrm{C}\), portanto
\[
  \Delta T=\Sexpr{Tmax-Tmin}\,^{\circ}\mathrm{C}.
\]
...
\[
  c=\frac{Q}{m\Delta T}
   =\frac{\Sexpr{P*tmax}}
          {\Sexpr{m}\cdot\Sexpr{Tmax-Tmin}}
   =\Sexpr{resp}\,\mathrm{cal/(g\,^{\circ}C)}.
\]

...
\begin{solution}
A potência térmica que precisa ser retirada pela água é \(\Sexpr{string}\) da potência elétrica da usina:
\[
  P_{\mathrm{term}}=\Sexpr{fator}\cdot\Sexpr{P}\,\mathrm{MW}.
\]
Como \(1\,\mathrm{MW}=1000\,\mathrm{kJ/s}\),
\[
  P_{\mathrm{term}}=\Sexpr{fator*1000*P}\,\mathrm{kJ/s}.
\]

Se \(\dot m\) é o fluxo de massa de água, então
\[
  P_{\mathrm{term}}=\dot m\,c\,\Delta T.
\]
Aqui,
\[
  \Delta T=28-\Sexpr{temp_media}
          =\Sexpr{28-temp_media}\,^{\circ}\mathrm{C}.
\]

Logo,
\[
  \dot m=
  \frac{\Sexpr{fator*1000*P}}
       {\Sexpr{calor_especifico}\cdot\Sexpr{28-temp_media}}
  =\Sexpr{resp}\,\mathrm{kg/s}.
\]

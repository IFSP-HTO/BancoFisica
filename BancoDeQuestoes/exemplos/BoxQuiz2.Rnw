...
SK_A <- ifelse(abs(SK(A)) < 0.2, "simétrica", ifelse(SK(A) >= 0.2, "assimétrica à direita", "assimétrica à esquerda"))
SK_B <- ifelse(abs(SK(B)) < 0.2, "simétrica", ifelse(SK(B) >= 0.2, "assimétrica à direita", "assimétrica à esquerda"))
...
questions[1] <- "As medianas das duas distribuições são aproximadamente iguais."
solutions[1] <- abs(trob(A, B)) < 0.5
explanations[1] <- if (solutions[1]) "As medianas das duas distribuições são aproximadamente iguais." else
  paste("A distribuição de ", c("A", "B")[(median(A) < median(B)) + 1],
    " tem em média valores mais altos do que a distribuição de ", 
    c("A", "B")[(median(A) >= median(B)) + 1], ".", sep = "")

outlier <- length(unlist(outlier)) > 0
questions[2] <- "Nenhuma das distribuições contém outliers."
solutions[2] <- !outlier
explanations[2] <- if (solutions[2])
  "Nenhuma das distribuições contém observações além dos limites $Q_1-1.5IQ$ e $Q_3+1.5IQ$." else "Há observações que estão além dos limites $Q_1-1.5IQ$ e $Q_3+1.5IQ$."

questions[3] <- "A dispersão da amostra A é claramente maior do que a dispersão da amostra B."
solutions[3] <- IQR(A)/IQR(B) > 1.5
explanations[3] <- paste("O intervalo interquartílico da amostra A é",
  ifelse(solutions[3], "", "\\\\textit{not}"), "claramente maior do que o da amostra B.")

i <- sample(1:2, 1)
j <- sample(1:3, 1)
questions[4] <- paste("A distribuição de ", c("A", "B")[i], " é ", 
                      c("aproximadamente simétrica", "assimétrica à direita", "assimétrica à esquerda")[j], ".", sep = "")
SK_i <- SK(list(A, B)[[i]])
solutions[4] <- switch(j, abs(SK_i) < 0.2,
                       SK_i >= 0.2,
                       SK_i <= -0.2)
explanations[4] <- paste("A distribuição de ", c("A", "B")[i], " é ",
                         ifelse(abs(SK_i) < 0.2, "aproximadamente simétrica.", 
                                ifelse(SK_i >= 0.2, "assimétrica à direita.", "assimétrica à esquerda.")))

questions[5] <- "Todas as afirmações anteriores são falsas"
solutions[5] <- sum(solutions[1:4])==0
explanations[5] <- " "
@

\begin{question}
A figura apresenta boxplots para duas amostras (A e B).
Quais das afirma\c{c}\~oes abaixo est\~ao corretas?  \emph{(Obs: As
  afirma\c{c}\~oes est\~ao aproximadamente corretas ou claramente erradas.)}

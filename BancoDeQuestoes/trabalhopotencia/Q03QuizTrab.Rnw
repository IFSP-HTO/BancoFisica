...
\begin{question}
(Ufsc) A figura mostra um bloco, de massa m = \Sexpr{massa} g, mantido encostado em uma mola comprimida de x = \Sexpr{compressao} cm. A constante elástica da mola é K = \Sexpr{const_elast} N/m. A mola é solta e empurra o bloco que, partindo do repouso no ponto A, atinge o ponto B, onde pára. No percurso entre os pontos A e B, a força de atrito da superfície sobre o bloco dissipa 20 por cento da energia mecânica inicial no ponto A.

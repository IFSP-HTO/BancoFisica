...
## Marcelo C.
## Parâmetros
xpasso <- sample(seq(from=1, to=5, by=0.5), 1)
x <- c(0, xpasso, 2*xpasso, 3*xpasso, 4*xpasso)
Fmax <- sample(seq(from=6, to=400, by=2), 1)
...
plot(x, y, type="o", col="blue", ylab="Força (N)", ylim=range(0, Fmax), yaxp=c(0,Fmax,2), xlab="Distância (m)", xaxp=c(0,4*xpasso,4))
#abline(h=c(0,Fmax/2,Fmax), v=0:4*xpasso, col="gray", lty=3)
abline(h=seq(0,Fmax,Fmax/4), v=seq(0,4*xpasso,xpasso), col="gray", lty=3)
@
\end{center}
\end{figure}
